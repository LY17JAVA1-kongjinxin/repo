...
<script type="text/javascript">
	function delpType(pid,pageNo){
		var flag=confirm("您确定要删除这条记录吗?");
		if(flag){
			window.location.href="/RJDZ/productType?type=del&pid="+pid+"&pageNo="+pageNo;
		}
	}
	
	function updpType(pid,pageNo){
		window.location.href="/RJDZ/productType?type=findById&pid="+pid+"&pageNo="+pageNo;
	}
</script>
</head>
<body>
				...
				<div class="box">
					...
					<div class="box-body table-responsive no-padding">
						...
						<table id="grid"
							...
							<tbody>
								<c:forEach items="${requestScope.list }" var="pType">
									<tr>
										<td>${pType.pid }</td>
										<td>${pType.pname }</td>
										<td>${pType.porder }</td>
										<td><button id='editrow' class='btn btn-success btn-sm' type='button' onclick="updpType(${pType.pid },${requestScope.pager.currentPage })">编辑</button>&nbsp
											<button id='delrow' class='btn btn-danger btn-sm'
												type='button' onclick="delpType(${pType.pid },${requestScope.pager.currentPage })">删除</button></td>
									</tr>
								</c:forEach>
							</tbody>
							<tr>
								<td colspan="4" align="right">
									<ul class="pager">
										<li><a href="/RJDZ/productType?type=findAll&pageNo=1">首页</a></li>
										<li><a href="/RJDZ/productType?type=findAll&pageNo=${requestScope.pager.prePageNo }">上一页</a></li>
										<li><a href="/RJDZ/productType?type=findAll&pageNo=${requestScope.pager.nextPageNo }">下一页</a></li>
										<li><a href="/RJDZ/productType?type=findAll&pageNo=${requestScope.pager.totalPage }">尾页</a></li>
