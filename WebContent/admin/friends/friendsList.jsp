...
<script type="text/javascript">
	function delFriends(fid,pageNo){
		var flag=confirm("您确定要删除这条记录吗?");
		if(flag){
			window.location.href="/RJDZ/friends?type=del&fid="+fid+"&pageNo="+pageNo;
		}
	}
	function updFriends(fid,pageNo){
		window.location.href="/RJDZ/friends?type=findById&fid="+fid+"&pageNo="+pageNo;
	}
				...
				<div class="box">
					...
					<div class="box-body table-responsive no-padding">
						...
						<table id="grid"
							...
							<tbody>
								<c:forEach items="${requestScope.list }" var="friends">
									<tr>
										<td>${friends.fid }</td>
										<td>${friends.fname }</td>
										<td>${friends.furl }</td>
										<td>${friends.ftime }</td>
										<td><button id='editrow' class='btn btn-success btn-sm' type='button' onclick="updFriends(${friends.fid },${requestScope.pager.currentPage })">编辑</button>&nbsp;
											<button id='delrow' class='btn btn-danger btn-sm' type='button' onclick="delFriends(${friends.fid },${requestScope.pager.currentPage })">删除</button></td>
									</tr>
								</c:forEach>
							</tbody>
							<tr>
								<td colspan="5" align="right">
									<ul class="pager">
										<li><a href="/RJDZ/friends?type=findAll&pageNo=1">首页</a></li>
										<li><a href="/RJDZ/friends?type=findAll&pageNo=${requestScope.pager.prePageNo }">上一页</a></li>
										<li><a href="/RJDZ/friends?type=findAll&pageNo=${requestScope.pager.nextPageNo }">下一页</a></li>
										<li><a href="/RJDZ/friends?type=findAll&pageNo=${requestScope.pager.totalPage }">尾页</a></li>
